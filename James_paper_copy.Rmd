...
We recreated this example from the James-Stein Bookdown to show how cricket is similar to baseball in it's utility for teaching a wide range of statistical concepts. With a new open source data set available, the statistical community and fans have a rich new data set to work with for both teaching and as fans.

Take the James-Stein estimator as an example. We choose batsmen who have face d aminimum of 400 balls in the 2019 and 2019/2020 seasons, and using their final strike rate as the "true" strike rate, we estmate at various points in the season what their final strike rate will be. Since T20 cricket is played on the international stage, and at various domestic tournaments which players from around the world are allowed to play in, we will just choose the 10%, 25%, and 50% of their totals balls faced for the season as these estimation points.
...
count_50 <- obs %>% summarise(median(count_50)) %>% pull()
  ...
  theme_minimal()
	...
	obs %>% 
  mutate(sigma2 = sum((mle_50-p_50_)^2)/count_50,
         JS=p_50_+(1-((N-3)*sigma2/(sum((mle_50-p_50_)^2))))*(mle_50-p_50_)) %>% 
